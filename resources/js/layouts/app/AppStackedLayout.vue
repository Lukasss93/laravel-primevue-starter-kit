...
const menuItems = [
    ...
    { label: 'Messages', icon: 'pi-comments', badge: '7' },
...
<template>
    <div class="resize-container-4 min-h-screen flex flex-col bg-surface-50 dark:bg-surface-950">
        <div class="bg-surface-900 py-4 px-8 lg:px-12 shadow flex items-center justify-between relative lg:static border-b border-surface-800 min-h-[68px] gap-8">
            <div class="flex items-center gap-4">
                ...
                <div class="text-white font-semibold text-lg">Trimzales</div>
            </div>
                ...
                <div class="flex-1 flex items-center gap-4 px-8 lg:px-4 py-4 lg:py-0 flex-col lg:flex-row">
                    <a
                        ...
                        :class="[
                            ...
                            activeItem === i ? 'bg-surface-800 border-surface-700 text-white' : 'border-transparent hover:bg-surface-800 hover:border-surface-700 text-surface-400 hover:text-white'
                        ]"
                        @click="setActiveItem(i)"
                    >
                        <i :class="[`pi ${item.icon} text-base! leading-normal!`, activeItem === i ? 'text-white' : 'text-surface-400']" />
                        <span class="font-medium">{{ item.label }}</span>
                        <Badge v-if="item.badge" :value="item.badge" severity="warning" class="md:block hidden ml-2" />
                    </a>
                </div>
                <div class="flex items-center gap-6 px-8 lg:px-0 py-4 lg:py-0 border-t lg:border-t-0 border-surface-800 flex-col lg:flex-row">
                    <div class="w-full lg:w-54">
                        <IconField icon-position="left" class="w-full">
                            <InputIcon class="pi pi-search text-surface-400" />
                            <InputText type="text" placeholder="Search" class="bg-surface-800! text-surface-400! border-surface-800! w-full py-2! px-4! pl-10! rounded-md!" />
                        </IconField>
                    </div>
                    <div class="flex items-center gap-4 w-full lg:w-auto">
                        <img src="https://fqjltiegiezfetthbags.supabase.co/storage/v1/render/image/public/block.images/blocks/avatars/circle/avatar-m-1.png" class="w-8 h-8 rounded-full shrink-0 cursor-pointer max-w-none" />
                        <div class="lg:hidden flex flex-col gap-1">
                            <div class="text-white font-medium">Robert Fox</div>
                            <div class="text-surface-400 text-sm">Developer</div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-surface-900 py-2 px-8 lg:px-12 border-b border-surface-200 dark:border-surface-800">
            <div class="flex items-center gap-4 overflow-x-auto">
                <a class="flex items-center gap-2 py-2 cursor-pointer">
                    <i class="pi pi-home text-base! leading-normal! text-surface-500 dark:text-surface-400" />
                </a>
                <i class="pi pi-angle-right text-sm! leading-normal! text-surface-300 dark:text-surface-600" />
                <a class="flex items-center py-2 cursor-pointer">
                    <span class="text-surface-900 dark:text-white font-medium">Dashboard</span>
                </a>
            </div>
        </div>
        <div class="flex flex-col flex-1">
            <div class="p-8 bg-surface-50 dark:bg-surface-950 flex flex-1">
                <div class="flex-1 bg-surface-0 dark:bg-surface-900 rounded-xl border-2 border-dashed border-surface-200 dark:border-surface-700" />
            </div>
        </div>
    </div>
</template>
